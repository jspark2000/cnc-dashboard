...
const generateRandomList = (length: number, minVal: number, maxVal: number): number[] => {
  return Array.from({ length }, () => Math.floor(Math.random() * (maxVal - minVal + 1)) + minVal);
};
...
const Charts = () => {
  ...
  return (
    <div>
      <div className="flex h-[250px] items-center mt-3">
        <div className="w-1/2 flex h-full items-center">
          <div className="w-1/3">
            <PieChart series={utilizationSeries} labels={['점유율']} title='점유율' />
          </div>
          <div className="w-1/3">
            <PieChart series={machineOccupancySeries} labels={['가동', '비가동']} title='장비 가동률' />
          </div>
          <div className="w-1/3">
            <PieChart series={occupancySeries} labels={['가공 가동', '비가공 가동']} title='가공 가동률' />
          </div>
        </div>
        <div className="w-1/2 h-full">
          <div className="h-1/2 flex">
            <div className="w-1/3">
              <GaugeChart series={availabilityGaugeSeries} label="가용성 (Availability)" />
            </div>
            <div className="w-1/3">
              <GaugeChart series={performanceGaugeSeries} label="성능 (Performance)" />
            </div>
            <div className="w-1/3">
          ...
          <div className="h-1/2 flex">
            <div className="w-1/3">
              <ColumnChart series={availabilityBarSeries} labels={["가공시간", "계획가공시간"]} />
            </div>
            <div className="w-1/3">
              <ColumnChart series={performanceBarSeries} labels={["생산수량", "계획수량"]} />
            </div>
            <div className="w-1/3">
              <ColumnChart series={qualityBarSeries} labels={["양품", "불량품"]} />
            </div>
          </div>
        </div>
      </div>
      <div className="items-center mt-3 w-full">
